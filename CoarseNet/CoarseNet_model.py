...
def conv_bn_prelu(bottom, w_size, name, strides=(1, 1), dilation_rate=(1, 1)):
    ...
    top = BatchNormalization(name='bn-' + name)(top)
    top = PReLU(alpha_initializer='zero', shared_axes=[1, 2],
                name='prelu-' + name)(top)
...
def CoarseNetmodel(input_shape=(400, 400, 1), weights_path=None, mode='train'):
    ...
    # multi-scale ASPP
    level_2 = conv_bn_prelu(conv_block3, (256, 3, 3), '4_1',
                            dilation_rate=(1, 1))
    ...
    level_3 = conv_bn_prelu(conv_block2, (256, 3, 3), '4_2',
                            dilation_rate=(4, 4))
    ...
    level_4 = conv_bn_prelu(conv_block2, (256, 3, 3), '4_3',
                            dilation_rate=(8, 8))
    ...
    enh_seg_img = Lambda(
        merge_concat, name='phase_seg_img'
    ...
    if mode == 'deploy':
        model = Model(
            inputs=[img_input, ],
            outputs=[enh_img, enh_img_imag, enh_img_real, ori_out_1,
                     ori_out_2, seg_out, mnt_o_out, mnt_w_out,
                     mnt_h_out, mnt_s_out])
    else:
        model = Model(
            inputs=[img_input, ],
            outputs=[ori_out_1, ori_out_2, seg_out, mnt_o_out,
                     mnt_w_out, mnt_h_out, mnt_s_out])
...
def train(input_shape=(400, 400), train_set=None,
          ...
          pretrain_dir=None, batch_size=1, test_set=None, learning_config=None,
          logging=None):
    ...
    plot_model(main_net_model, to_file=output_dir + '/model.png',
               show_shapes=True)

    main_net_model.compile(optimizer=learning_config,
                           loss={'seg_out': segmentation_loss,
                                 'mnt_o_out': orientation_output_loss,
                                 'mnt_w_out': orientation_output_loss,
                                 'mnt_h_out': orientation_output_loss,
                                 'mnt_s_out': minutiae_score_loss
                                 },
                           loss_weights={'seg_out': .5,
                                         'mnt_w_out': .5,
                                         'mnt_h_out': .5,
                                         'mnt_o_out': 100.,
                                         'mnt_s_out': 50.
                                         },
                           metrics={'seg_out': [seg_acc_pos, seg_acc_neg,
                                                seg_acc_all],
                                    'mnt_o_out': [mnt_acc_delta_10, ],
                                    'mnt_w_out': [mnt_mean_delta, ],
                                    'mnt_h_out': [mnt_mean_delta, ],
                                    'mnt_s_out': [seg_acc_pos, seg_acc_neg,
                                                  seg_acc_all]
                                    })
    ...
    for epoch in range(1000):
        ...
        data = load_data((img_name, folder_name, img_size), tra_ori_model,
                         rand=True, aug=0.7, batch_size=batch_size)
        for i, train in enumerate(data):
            ...
            if i % (2 / batch_size) == 0:
                ...
                logging.info("%s", " ".join(
                    ["%s:%.4f\t" % (x) for x in zip(
                        main_net_model.metrics_names, loss)]))
...
def deploy_with_GT(deploy_set, output_dir, model_path,
    ...
    for i, test in enumerate(
        ...
        dir_map, fre_map = get_maps_STFT(texture_img, patch_size=64,
                                         block_size=16, preprocess=True)
            ...
            if FineNet_path is not None:
                for idx_minu in range(mnt_nms.shape[0]):
                    try:
                        ...
                        ret = np.empty((patch_minu.shape[0],
                                        patch_minu.shape[1], 3),
                                       dtype=np.uint8)
                        ret[:, :, 0] = patch_minu
                        ret[:, :, 1] = patch_minu
                        ret[:, :, 2] = patch_minu
